...
export class Enemy extends NotchSprite {
    ...
    declare SpriteTemplate: any;
    declare Facing: number;

    constructor(world: any, x: number, y: number, dir: number, type: number, winged: boolean) {
    ...
    SubDraw(context, camera) {
        Mario.NotchSprite.prototype.Draw.call(this, context, camera);
    }
    ...
    Draw(context, camera) {
        ...
        if (this.Winged) {
            ...
            if (this.Type === Enemy.RedKoopa && this.Type === Enemy.GreenKoopa) {
                ...
                context.scale(this.XFlip ? -1 : 1, this.YFlip ? -1 : 1);
                context.translate(this.XFlip ? -320 : 0, this.YFlip ? -240 : 0);
                context.drawImage(this.Image, (((this.WingTime / 4) | 0) % 2) * 16, 4 * 32, 16, 32, this.XFlip ? 320 - xPixel - 24 : xPixel - 8, this.YFlip ? 240 - yPixel : yPixel - 8, 16, 32);
                context.restore();
            } else {
                context.save();
                context.scale(this.XFlip ? -1 : 1, this.YFlip ? -1 : 1);
