...
export class MapState extends GameState {
    ...
    GenerateLevel() {
        ...
        const n0 = new Mario.ImprovedNoise((Math.random() * 9223372036854775807) | 0);
        const n1 = new Mario.ImprovedNoise((Math.random() * 9223372036854775807) | 0);
        const dec = new Mario.ImprovedNoise((Math.random() * 9223372036854775807) | 0);

        const width = 320 / 16 + 1;
        const height = 240 / 16 + 1;
        this.Level = [];
        this.Data = [];

        const xo0 = Math.random() * 512;
        const yo0 = Math.random() * 512;
        const xo1 = Math.random() * 512;
        const yo1 = Math.random() * 512;
    ...
    IsRoad(x, y) {
        if (x < 0) x = 0;
        if (y < 0) y = 0;
        if (this.Level[x][y] === Mario.MapTile.Road) return true;
        if (this.Level[x][y] === Mario.MapTile.Level) return true;
        return false;
    }
    ...
    TryWalking(xd: number, yd: number) {
        const x = (this.XMario / 16) | 0;
        const y = (this.YMario / 16) | 0;
        const xt = x + xd;
        const yt = y + yd;
    ...
    LevelWon() {
        const x = this.XMario / 16;
        const y = this.YMario / 16;
        if (this.Data[x][y] === -2) {
            this.NextWorld();
            return;
        }
